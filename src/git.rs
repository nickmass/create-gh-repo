...
pub fn get_repo_name(target_dir: Option<&str>) -> Result<String> {
    ...
    let path = try!(repo.workdir().ok_or(Error::RepositoryBare));
    path.file_name().ok_or(Error::RepositoryBare)
        .map(|x| {x.to_string_lossy().to_owned().to_string()})
}

fn set_upstream(repo: &mut Repository, local_branch: &str, remote_branch: &str)
    -> Result<()> {
...
pub fn push(target_dir: Option<&str>, username: Option<&str>, password: Option<&str>)
    -> Result<String> {
    ...
    let repo = try!(find_repository(target_dir));
    {
        ...
        cbs.transfer_progress(|_| {
            true
        });
        cbs.sideband_progress(|_| {
            true
        });
        ...
        try!(remote.push(&["refs/heads/master:refs/heads/master"], Some(PushOptions::new().remote_callbacks(cbs))));
    }
    let mut repo = repo;
